...
function slider() {
    ...
    $.fn.Slider = function (options) {
        ...
        var slider = {}
            , el, windowWidth, windowHeight, init, props;
        ...
        init = function () {
                ...
                slider.settings.slideWidth = parseInt(slider.settings.slideWidth, 0);
        ...
        var setup = function () {
            ...
            // store a namspace reference to .sl-viewport
        ...
        var start = function () {
                ...
                if (slider.settings.infiniteLoop && slider.settings.mode != 'fade' && !slider.settings.ticker) {
                    ...
                    var sliceAppend = slider.children.slice(0, slice).clone().addClass('sl-clone');
                    var slicePrepend = slider.children.slice(-slice).clone().addClass('sl-clone');
        ...
        el.redrawSlider = function () {
                ...
                slider.children.add(el.find('.sl-clone')).width(getSlideWidth());
                // adjust the height
...
function carousel() {
    ...
    //    clientLogSlider();
    //
    //    function clientLogSlider() {
    //        if ($('.client-logo').length > 0) {
    //            var width = (window.innerWidth > 0) ? window.innerWidth : screen.width;
    //            var mySlider = $('.client-logo').Slider({
    //                minSlides: (width < 768) ? 2 : 4
    //                , maxSlides: (width < 768) ? 2 : 4
    //                , slideWidth: 250
    //                , auto: true
    //                , slideMargin: 0
    //                , infiniteLoop: false
...
function popover() {
    $.fn.popover = function (options) {
        ...
        if (typeof (options) === 'object' || !options) {
            ...
            opts = $.extend({}, $.fn.popover.defaults, options);
        }
    ...
    var CONST = {
        dataKey: '__popdata'
        , template: '<div class="popover">' + '<div class="arrow"></div>' + '<div class="popover_title"></div>' + '<div class="popover_content"></div>' + '</div>'
        , idPrefix: 'flyout'
        , directionClasses: {
            top: 'top'
            , right: 'right'
    ...
    function __show(data) {
        ...
        if (data.flyoutId) {
            ...
            if (!data.opts.title) {
                ...
                $flyout.find('.popover_title').hide();
            }
            var titles1, con;
            if (data.opts.val === true) {
                titles1 = $this.attr('data-title');
                con = $this.attr('data-content');
            }
            /* set title and contents */
            var title = (typeof (data.opts.title) === 'function' ? ((titles1).call($this)) : titles1);
            $flyout.attr({
                id: id
            }).find('.popover_title').text(title);
            var contents = (typeof (data.opts.content) === 'function' ? ((con).call($this)) : con);
            if (data.opts.html === true) {
                $flyout.find('.popover_content').html(contents);
            }
            else {
                $flyout.find('.popover_content').text(contents);
            }
        ...
        /* set the placement */
        $flyout.removeClass(CONST.directionClasses.top + ' ' + CONST.directionClasses.right + ' ' + CONST.directionClasses.bottom + ' ' + CONST.directionClasses.left);
...
function showPopover() {
    popover();
    $('#trigger_top').popover();
    $('#trigger_right').popover({
        placement: 'right'
    });
    $('#trigger_bottom').popover({
        placement: 'bottom'
    });
    $('#trigger_left').popover({
        placement: 'left'
    });
}
...
function dropdown() {
    ...
    $(".dropdown-toggle").each(function (index) {
        $(this).on("click", function () {
            var id = $(this).attr('id');
            $(".dropdown-menu").each(function (index1) {
                drop_target = $(this).attr('drop-area');
                if (typeof drop_target !== typeof undefined && drop_target !== false) {
                    if (id == drop_target) {
                        drop_elm = this;
                        return drop_elm;
                    }
            ...
            $(drop_elm).toggleClass('open-drop');
        });
    });
}
